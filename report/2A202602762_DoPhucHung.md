# Báo cáo Vai trò — Day 10: Data Pipeline & Data Observability
...
### 2.1 Phần việc sở hữu

| Module / Deliverable | File / Hàm phụ trách | Input | Output | Trạng thái |
|---|---|---|---|---|
| Quality Gate GX 1.x | `src/observability/quality.py` — `run_data_quality_checks`, `_build_expectations` | DataFrame (clean / corrupted / repaired) | `data/quality/{baseline,corrupted,repaired}_quality_report.json` | ✅ Hoàn thành |
| Freshness SLA | `build_freshness_report`, `_stale_stats` | DataFrame có `published`, `age_days` | `data/quality/freshness_report.json` và biến thể cho corrupted/repaired | ✅ Hoàn thành |
| Evaluation set | `src/evaluation/testset.py` — `build_test_set` | Clean DataFrame | `data/eval/test_set.json` (10 câu, 4 nhóm) | ✅ Hoàn thành |
...
### 2.2 Việc hỗ trợ ngoài phạm vi chính
...
**Chi tiết `corrupted_quality_report.json`:** 4 expectation fail — `expect_column_values_to_be_unique(paper_id)` (6 giá trị trùng), `expect_column_value_lengths_to_be_between(title)` (4), `expect_column_value_lengths_to_be_between(summary)` (3), và `expect_column_values_to_not_match_regex(summary)` (4). Danh sách này đồng thời là `trigger_reasons` kích hoạt auto-repair.
...
**Silent failure:** agent vẫn trả lời trơn tru trên dữ liệu hỏng. Phần của mình tạo ra các tín hiệu độc lập với agent để phát hiện dữ liệu xấu trước khi nó vào vector store (Quality Gate, Freshness), đồng thời dựng một thước đo cố định (test set) để lượng hóa thiệt hại.
...
- **Expectations:** 4 bắt buộc (row count 5–5000; not null `paper_id`/`title`/`text_for_embedding`; unique `paper_id`; summary ≥ 30 ký tự) và 2 mở rộng: title ≥ 8 ký tự, summary không khớp regex `(?:�|[#@!%&~]{3,}|0x[0-9A-Fa-f]{6,})`. Hai expectation mở rộng rất quan trọng — nếu thiếu, `truncate_title` và `inject_noise` sẽ lọt qua gate.
- **Cột list:** trên bản sao dùng để validate, `authors`/`categories` được đổi sang chuỗi để pandas engine của GX xử lý như text thông thường.
...
- **Test set:** lọc paper có tiêu đề không chứa `'` (vì `qa.py` trích tiêu đề bằng regex `'...'`), sắp theo ngày, chọn 10 vị trí trải đều từ mới nhất tới cũ nhất, gán lần lượt các loại câu hỏi. Ground truth dùng đúng format mà `qa.py` trả về.
...
| Phương án | Mô tả |
|---|---|
| (1) | `success = GX pass AND is_fresh` |
| (2) ✅ Chọn | `success` chỉ phản ánh GX; Freshness là tín hiệu riêng (`freshness.is_fresh`, `status`) |
...
**Bằng chứng:** Baseline hiện có 1/24 bài 181 ngày tuổi (sát ngưỡng). `test_freshness_report_on_clean_data` cố định `RUN_DATE = 2026-09-25` để kết quả kiểm thử không trôi theo ngày chạy.
...
| Mục | Nội dung |
|---|---|
| **Triệu chứng** | Khi chạy pytest, GX in cảnh báo: `UserWarning: This pattern is interpreted as a regular expression, and has match groups. To actually get the groups, use str.extract.` Ngoài ra, mỗi lần validate log bị ngập bởi thanh tiến trình `Calculating Metrics: ...` |
| **Tái hiện** | `uv run pytest -q` với regex ban đầu `(?:|[#@!%&~]{3,}|0x...)`, và `uv run python script/run_phase1.py` |
...
| **Xác minh sau sửa** | `uv run pytest -q` → `15 passed` không còn warning; log Phase 1 chỉ còn các dòng `[phase1] ...` |
...
1. Raw JSON từ Crossref được parse thành record, rồi làm sạch thành bảng có `text_for_embedding`. Bảng này phải qua gate GX trước; pass mới được embed bằng MiniLM và ghi vào ChromaDB.
...
- `inject_noise` → GX regex fail (4 dòng) → eval_005 trả lời bắt đầu bằng `~~~ 0xDEADBEEF ...`, F1 0.84, judge 3/5: vẫn tính là "đúng" dù câu trả lời lẫn rác.
- Repair từ raw → GX 8/8, FRESH → breakdown mọi `question_type` về 1.0.

**Corruption ảnh hưởng rõ nhất:** blank summary kết hợp drop latest — eval_001 vừa mất tài liệu gốc, vừa được trả lời bằng bài có summary rỗng, nên câu trả lời rỗng (F1 0, judge 1).
...
Sinh test set phủ toàn bộ 24 paper × 4 loại câu hỏi (96 câu) và so lại delta của `stale_date`. Cách đo: F1 của nhóm `date` phải giảm tương ứng với 6/24 bài bị lùi ngày.
